...
import { updateExpression, updateHistory, setInvalidInputFormat, setInputValue } from "@store/actions/actions";

import { Calculator, AddToStackCommand, GetFromStackCommand, AddCommand, SubtractCommand, MultiplyCommand, DivideCommand, RemainderCommand} from '@utils/calculation';
...
function CalculatorFC () {
        ...
        expression.forEach(item => {
            if (!isNaN(item)) {
                ...
                while (stack.length > 0) {
                    ...
                    if (isOperator(peekedItem) && getOperatorePriority(peekedItem) >= getOperatorePriority(item)) {
    ...
    const calculate = (expression) => {
        ...
        expression.forEach(item => {
            if (!isNaN(item)) {
                ...
                calculator.executeCommand(new AddCommand(+calculator.executeCommand(new GetFromStackCommand())));
    ...
    const onEqual = () => {
        ...
        if (!Number.isNaN(result) && finalFormula.length !== 1) {
            result = result === 0 ? "" : (result % 1 !== 0 ? result.toFixed(3) : result);

            let newHistoryList = [...history, {expression: [...expression, input].join(""), result}];
    ...
    return (
        <CalculatorContainer>
            ...
            <CalculatorInnerContainer>
                <ControlPanel onSetHistoryState={() => setHistoryState(!historyState)}/>
