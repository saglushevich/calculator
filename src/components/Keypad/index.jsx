...
import { updateExpression, setInvalidInputFormat, setInputValue } from "@store/actions/actions";
...
function Keypad ({onEqual}) {
    ...
    const buttons = KEYPAD_BUTTONS.map(item => {
        switch (item.id) {
            case "clear": 
                return <KeypadButton key={item.id} value={item.value} onSelectElement={onClearExpression}/>
            case "equals":
                return <KeypadButton key={item.id} value={item.value} onSelectElement={onEqual}/>
            case "changeSign":
                return <KeypadButton key={item.id} value={item.value} onSelectElement={onChangeSign}/>
            default:
                return <KeypadButton onSelectElement={() => onSelectElement(item.value)} key={item.id} value={item.value}/>
        }
    })
